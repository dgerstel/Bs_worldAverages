...
ResDir = "../Results/"
outputFile = ResDir + thisVersion + thisYear +"_Results_GsDGs_and_PhisDGs.txt"
...
val["D0"]['Gs']               = GsD0  # 0.6930
val["D0"]['Gs_estat']         = GsD0etot # 0.017529
val["D0"]['Gs_esyst']         = 0.
val["D0"]['DGs']              = 0.163
val["D0"]['DGs_estat']        = sqrt((.064**2+.065**2)/2) # = 0.06450193
...
#--------------------------------------------------------------------------------------------------------
tauin["Jpsif0"]["publi"]    = "Jpsif0 from CDF, D0. JpsiPiPi from LHCb 1fb-1 and CMS 19.7fb-1 (2017)"
tauin["Jpsif0"]["tau"]      = 1.668051 # was 1.65765
tauin["Jpsif0"]["tau_etot"] = 0.024189 # was 0.03188
#--------------------------------------------------------------------------------------------------------
# Details on Jpsif0, JpsiPiPi, June 2017:
#--------------------------------------------------------------------------------------------------------
# Summer2017 average. Only new results = http://cms-results.web.cern.ch/cms-results/public-results/preliminary-resu\
# lts/BPH-13-008/index.html
# 19.7fb-1
# Bs2 JpsiPiPi lifetime

# clight = 299.792458;
# ctauCMS = 504.3;
# ctauCMSestat = 10.5;
# ctauCMSesyst = 3.7;
# tauH_CMS = ctauCMS/clight
# tauH_CMS_estat= ctauCMSestat/clight
# tauH_CMS_esyst= ctauCMSesyst/clight
# tauH_CMS_etot= sqrt(tauH_CMS_estat**2+tauH_CMS_esyst**2)

# print "tauH CMS =", tauH_CMS, "+/-", tauH_CMS_etot
# # tauH CMS = 1.68216373208 +/- 0.0371351380699


# Jpsif0 from CDF, D0 and LHCb JpsiPiPi 1fb-1
# a= 1.65765
# sa = 0.03188
...
########### Average of old (Jpsif0 from CDF, D0 and LHCb JpsiPiPi 1fb-1) with June 2017 CMS JpsiPiPi:
# err_tauH = sqrt(1/(1/sa**2+1/sb**2))
# tauH = err_tauH**2*(a/sa**2+b/sb**2)
# print "tauH =", tauH, "+/-", err_tauH
# # tauH = 1.66805103502 +/- 0.0241890384409
#--------------------------------------------------------------------------------------------------------



tauin["FS"]["publi"]      = "tauFS, OS, COMBOS, including D0 DsMuX and LHCb DsMuNu LakeLouise 2017, https://arxiv.org/abs/1705.03475, tau(Bs -> Ds(*) mu nu ) = 1.547 pm 0.013(stat) pm 0.010(syst) pm 0.004(tauB0) ps. Including CMS note BPH-13-008, 2017"
tauin["FS"]["tau"]        = 1.526
...
for i in range(len(coords)):
    ...
    if (i == 2):  # flavour-specific -- draw at an angle
	    plt.text(coords[i][0], coords[i][1], labels[i], verticalalignment='center', 
                     horizontalalignment='center', color=colors[i], fontsize=25, rotation=-42)
    else:
